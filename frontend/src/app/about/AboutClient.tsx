...
/* ── Hardcoded defaults (used when Keystatic has no content yet) ─────────── */

const DEFAULTS = {
    heroTagline: 'Passionately shaping backyards into timeless designs',
    manifesto:
    ...
    beliefs: [
        { word: 'Authenticity', pill: 'A fundamental trust in materials' },
        { word: 'Aesthetic',    pill: 'Curated designs that inspire' },
        { word: 'Advocacy',     pill: 'Standards you can trust' },
    ],
    valuesHeading:
    ...
    founderName: 'Hassan Bari',
    founderRole: 'CEO & Founder',
    founderImage: '/Hassan-Bari-(Aquavida).avif',
}
...
function AboutHeroSection({ d }: { d: typeof DEFAULTS }) {
    return (
...
function ApproachSection({ d }: { d: typeof DEFAULTS }) {
    return (
...
function MoreAboutSections({ d }: { d: typeof DEFAULTS }) {
    const beliefs = d.beliefs.length > 0 ? d.beliefs : DEFAULTS.beliefs
    const values  = d.values.length  > 0 ? d.values  : DEFAULTS.values
                    ...
                    <div className="flex flex-col gap-8 md:gap-4">
                        {beliefs.map((b, i) => (
                            <motion.div
                                ...
                                viewport={{ once: true }}
                                className={`relative w-full pb-8 md:pb-0 ${
                                    i % 2 === 0
                                        ? 'md:self-end md:mr-[10%]'
                                        : 'md:self-center'
                                }`}
                            >
                                <h2 className={`text-[13vw] md:text-[clamp(4rem,9vw,13rem)] font-light text-[#121212] leading-none tracking-tighter ${
                                    i % 2 === 0 ? 'ml-[16vw] md:ml-[32vw]' : 'ml-[8vw] md:ml-[7vw]'
                                }`}>
                                    {b.word}
                                </h2>
                                <FloatingPill
                                    ...
                                    className={
                                        i === 0
                                            ? 'top-full mt-2 left-0 md:top-[0%] md:mt-0 md:left-auto md:right-[-6vw]'
                                            : i === 1
                                                ? 'top-full mt-2 right-0 md:top-0 md:mt-0 md:right-auto md:left-0'
                                                : 'top-full mt-2 left-0 md:top-auto md:mt-0 md:left-auto md:bottom-0 md:right-[-8vw]'
                                    }
                                />
                            </motion.div>
                        ))}
...
export default function AboutClient({ data }: { data?: AboutPageData }) {
    const d: typeof DEFAULTS = {
        heroTagline:         data?.heroTagline         || DEFAULTS.heroTagline,
        manifesto:           data?.manifesto           || DEFAULTS.manifesto,
        approachQuote:       data?.approachQuote       || DEFAULTS.approachQuote,
        approachDescription: data?.approachDescription || DEFAULTS.approachDescription,
        beliefs:             data?.beliefs?.length ? data.beliefs as typeof DEFAULTS.beliefs : DEFAULTS.beliefs,
        valuesHeading:       data?.valuesHeading       || DEFAULTS.valuesHeading,
        values:              data?.values?.length  ? data.values  as typeof DEFAULTS.values  : DEFAULTS.values,
        founderBio:          data?.founderBio          || DEFAULTS.founderBio,
        founderName:         data?.founderName         || DEFAULTS.founderName,
        founderRole:         data?.founderRole         || DEFAULTS.founderRole,
        founderImage:        data?.founderImage        || DEFAULTS.founderImage,
    }

    return (
        <main className="min-h-screen bg-[#05070A] text-white selection:bg-white/20">
            <AboutHeroSection d={d} />
            <ApproachSection d={d} />
            <MoreAboutSections d={d} />
        </main>
    );
}
